 ...
 *
 * An example usage includes compressing a SceneGeometryParams into a {@link scene!SceneGeometryCompressedParams | SceneGeometryCompressedParams} using the
 * {@link scene!compressGeometryParams | compressGeometryParams} function.
 *
 ...
 * generated for the {@link constants!TrianglesPrimitive}, and a positionsDecompressMatrix is included to de-quantize
 * the positions within the Viewer.
 *
 * This library provides a set of functions that are used internally within
 * {@link scene!SceneModel.createGeometry | SceneModel.createGeometry} implementations to
 ...
 * ````
 *
 * In the next example, we'll again use {@link scene!compressGeometryParams | compressGeometryParams} to compress
 * a {@link scene!SceneGeometryParams | SceneGeometryParams} into a
 * {@link scene!SceneGeometryCompressedParams | SceneGeometryCompressedParams}, which we'll then use to
 * create a compressed geometry within a {@link scene!SceneModel | SceneModel}.
 *
 * ````javascript
