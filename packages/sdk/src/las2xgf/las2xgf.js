...
const {las2xgf, _SAVED_XGF_VERSIONS, _DEFAULT_SAVED_XGF_VERSION} = require("./las2xgf_core.cjs.js");
...
program
    .description(`CLI to convert a LAS/LAZ file into a xeokit XGF SceneModel file and/or a JSON DataModel file`)
    .option('-i, --input [file]', 'path to input LAS/LAZ model file (required)')
...
function logInfo(msg) {
    if (options.log) {
        console.log(`[las2xgf] ${msg}`);
    }
}

function logError(msg) {
    console.error(`[las2xgf] ${msg}`);
}
...
try {
    ...
    las2xgf({
        ...
        if (options.log) {
            ...
            logInfo("Input LAS/LAZ file size: " + (sourceFileSizeBytes / 1000).toFixed(2) + " kB");
            logInfo("Output XGF file size: " + xgfSize + " kB");
            logInfo("LAS/LAZ->XGF compression ratio: " + compressionRatio);
