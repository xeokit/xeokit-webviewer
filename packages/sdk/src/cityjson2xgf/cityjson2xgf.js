...
const {cityjson2xgf, _SAVED_XGF_VERSIONS, _DEFAULT_SAVED_XGF_VERSION} = require("./cityjson2xgf_core.cjs.js");
...
program
    .description(`CLI to convert a CityJSON file into a xeokit XGF SceneModel file and/or a JSON DataModel file`)
    .option('-i, --input [file]', 'path to input CityJSON model file (required)')
...
function logInfo(msg) {
    if (options.log) {
        console.log(`[cityjson2xgf] ${msg}`);
    }
}

function logError(msg) {
    console.error(`[cityjson2xgf] ${msg}`);
}
...
try {
    ...
    const cityJSONSrc = options.input;
    ...
    const basePath = getBasePath(cityJSONSrc);
    const fileData = fs.readFileSync(cityJSONSrc);

    logInfo(`\nReading CityJSON file: ${cityJSONSrc}`);
    ...
    cityjson2xgf({
        ...
        if (options.log) {
            ...
            logInfo("Input CityJSON file size: " + (sourceFileSizeBytes / 1000).toFixed(2) + " kB");
            logInfo("Output XGF file size: " + xgfSize + " kB");
            logInfo("CityJSON->XGF compression ratio: " + compressionRatio);
